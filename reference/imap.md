...
IDLE alone reports only the SELECTed mailbox, so coverage of the remaining subscribed folders comes from `NOTIFY SET` (RFC 5465 Section 3), registered by `register_notify` once per IDLE round on the same connection: `(selected (MessageNew MessageExpunge FlagChange))` plus the same events under a `mailboxes` filter naming every other watched folder. Events on those arrive as STATUS responses (Section 4), which `map_idle_event` already turns into a per-folder invalidation, and `absorb_idle_event` is unaffected because the server sends FETCH/VANISHED only for the selected mailbox. The registration asks for no fetch attributes (Section 5.2 permits them only on `selected`, and the changes stream refetches anyway) and no initial STATUS snapshot, which would invalidate every watched scope immediately after subscribe. A server without NOTIFY, or one that rejects the registration, falls back to watching only the SELECTed mailbox and logs a warning naming how many folders that leaves unpushed - the silence is otherwise invisible.
...
The IDLE connection is dialed via `dial_idle` and never enters the pool's idle list, so `dial_idle` hands back an `Arc` and registers a `Weak` on the pool. `Pool::close` upgrades it and LOGOUTs it under the same `command_timeout` budget as the parked members; a redial replaces the registration. Without it `Account::close()` could return with the IDLE session still logged in.
