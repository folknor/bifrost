...
Still absent: a pending-request map that RESOLVES a caller's future when its response frame arrives. That needs an await-side door (`send_ws` returning a receiver) and a reader that routes to it; both would be unused surface today, since the only production consumer of the WS read stream is the push reader, which ignores `Response` frames. The correlation DATA now exists end to end, so adding the map is a local change when a caller needs one.
