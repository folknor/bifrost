...
macro_rules! calendar_setters {
    ($t:ty) => {
        impl $t {
            ...
            pub(crate) fn is_visible(&mut self, is_visible: bool) -> &mut Self {
                self.is_visible = Some(is_visible);
                self
            }

            pub(crate) fn include_in_availability(
                &mut self,
            ...
            pub(crate) fn default_alerts_with_time(
                &mut self,
            ...
            pub(crate) fn default_alerts_without_time(
                &mut self,
                alerts: Option<HashMap<String, Alert>>,
            ) -> &mut Self {
