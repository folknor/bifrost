...
pub(crate) fn crypto_provider() -> Arc<CryptoProvider> {
    ...
    #[cfg(not(any(feature = "aws-lc-rs", feature = "ring")))]
    {
        CryptoProvider::get_default()
            .cloned()
            .expect("No rustls crypto provider configured. When using the `rustls-no-provider` feature, a crypto provider must be installed before using bifrost_smtp. For example:\n\n    use rustls::crypto::CryptoProvider;\n    CryptoProvider::install_default(your_provider).expect(\"Failed to install crypto provider\");\n")
    }
}
