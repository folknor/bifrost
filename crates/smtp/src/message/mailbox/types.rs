...
///
/// ```
/// # use bifrost_smtp::{Address, message::Mailbox};
/// # use std::error::Error;
...
///
/// ```
/// # use bifrost_smtp::message::Mailbox;
/// # use std::error::Error;
/// # fn main() -> Result<(), Box<dyn Error>> {
...
impl Mailboxes {
    ...
    ///
    /// ```
    /// use bifrost_smtp::message::Mailboxes;
...
mod test {
    ...
    fn format_address_with_empty_name() {
        assert_eq!(
            format!(
                "{}",
                Mailbox::new(Some(String::new()), "kayo@example.com".parse().unwrap())
            ),
            "kayo@example.com"
        );
    }
