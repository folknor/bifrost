...
fn main() {
    ...
    let html = html! {
        head {
            title { "Hello from Bifrost SMTP!" }
            style type="text/css" {
                "h2, h4 { font-family: Arial, Helvetica, sans-serif; }"
            }
        }
        div style="display: flex; flex-direction: column; align-items: center;" {
            h2 { "Hello from Bifrost SMTP!" }
            // Substitute in the name of our recipient.
            p { "Dear " (recipient) "," }
            p { "This email was sent with Bifrost SMTP, a mailer library for Rust!"}
            p {
                "This example uses "
    ...
    let email = Message::builder()
        ...
        .subject("Hello from Bifrost SMTP!")
        .multipart(
            MultiPart::alternative() // This is composed of two parts.
                .singlepart(
                    SinglePart::builder()
                        ...
                        .body(String::from(
                            "Hello from Bifrost SMTP! A mailer library for Rust",
