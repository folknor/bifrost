...
//! flag means the method will reach the network, and CardDAV's `DavTransport`
...
use crate::account::CardDavAccount;
use crate::client::{CardDavClient, DavResponse, DavTransport};
...
impl DavTransport for NeverCalledTransport {
    fn send(
        ...
        panic!("an unsupported CardDAV method reached the wire");
    }
}

fn account() -> CardDavAccount {
    let client =
        CardDavClient::with_transport("https://dav.example.test", Arc::new(NeverCalledTransport));
...
async fn every_false_pim_flag_refuses_without_touching_the_wire() {
    ...
    // Contacts. All true on this account, so these are the true-direction
    // no-ops the macro skips - kept so a flag flipped to false starts being
    // checked without anyone having to remember to add a line.
    refuses!(
        caps,
        address_books_list,
...
#[test]
fn the_carddav_snapshot_advertises_both_answers() {
    let caps = crate::capabilities::carddav_capabilities();
    assert!(caps.pim_methods.contacts_list, "some flag is true");
    assert!(!caps.pim_methods.calendars_list, "some flag is false");
}
