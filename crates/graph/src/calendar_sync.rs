...
fn category_color_to_hex(color: &str) -> Option<String> {
    crate::preset_colors::preset_to_hex(color).map(|(bg, _)| bg.to_string())
}
...
pub async fn graph_list_calendars(client: &GraphClient) -> Result<Vec<GraphCalendarInfo>, String> {
...
pub async fn graph_sync_calendar_events(
    ...
    loop {
        ...
        let page: ODataCollection<GraphEvent> = if current_url.starts_with("http") {
            client.get_absolute(&current_url).await?
        } else {
            client.get_json(&current_url).await?
        };
...
pub async fn graph_update_event(
    ...
    let fetched: GraphEvent = client
        .get_json(&format!(
            "{me}/events/{enc_event_id}?$select={EVENT_SELECT}"
        ))
        .await?;
...
fn map_graph_event(event: GraphEvent) -> Result<GraphCalendarEvent, String> {
    ...
    // Anchor end_time to start_time + date_delta*86400 to keep the duration
...
/// local times are resolved through the local DST resolver.
...
fn parse_graph_datetime(
    ...
    if is_all_day {
        ...
        return Ok(
            crate::time::resolve_local_to_timestamp(naive, &chrono::Local)
    ...
    if let Some(tz) = resolve_graph_tz(&dt.time_zone)
        && let Some(ts) = crate::time::resolve_local_to_timestamp(naive, &tz)
    {
        return Ok(ts);
    }
...
mod tests {
    ...
    fn map_event_all_day_spanning_dst_keeps_exact_day_count() {
        ...
        // 47*3600s. The fix anchors end_time to start + date_delta*86400 so
