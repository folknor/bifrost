...
/// Runtime mapping between opaque Graph folder IDs and stable folder IDs.
#[derive(Debug, Clone)]
...
impl FolderMap {
    ...
    pub fn build(
        ...
        // First pass: build opaque Graph ID -> stable folder ID map for parent resolution.
        let graph_to_folder_id: HashMap<&str, String> = all_folders
            ...
            .map(|folder| {
                let folder_id = if let Some(&(lid, _)) = resolved_wellknown.get(&folder.id) {
                    ...
                    graph_folder_storage_id(&folder.id)?
                };
        ...
        for folder in all_folders {
            ...
            let mapping =
                if let Some(&(folder_id, folder_name)) = resolved_wellknown.get(&folder.id) {
                    ...
                    FolderMapping {
                        folder_id: graph_folder_storage_id(&folder.id)?,
    ...
    /// Resolve a stable folder ID to an opaque Graph folder ID.
    ...
    pub fn get_folder_and_label_ids_for_message(
        ...
        for cat in categories {
            ids.push(graph_category_storage_id(cat)?);
        }
        Ok(ids)
    }
...
fn graph_folder_storage_id(graph_id: &str) -> Result<String, String> {
    if graph_id.is_empty() || graph_id.chars().any(char::is_control) {
        return Err("Graph folder ID cannot be empty or contain control characters".to_string());
    }
    Ok(format!("graph-{graph_id}"))
}

fn graph_category_storage_id(category: &str) -> Result<String, String> {
    if category.is_empty() || category.chars().any(char::is_control) {
        return Err("Graph category cannot be empty or contain control characters".to_string());
    }
    Ok(format!("cat:{category}"))
}

fn graph_well_known_aliases() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ("inbox", "INBOX", "Inbox"),
        ("drafts", "DRAFT", "Drafts"),
        ("sentitems", "SENT", "Sent"),
