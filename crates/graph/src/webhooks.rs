...
pub async fn delete_subscription(
    ...
    // expired, in which case Graph returns 404.
    let server_result = client.delete(&path).await;
...
pub async fn list_subscriptions(client: &GraphClient) -> Result<Vec<SubscriptionResponse>, String> {
    let response: SubscriptionListResponse = client.get_json("/subscriptions").await?;
    Ok(response.value)
}
...
/// Check subscriptions and renew any that are expiring soon.
///
/// - Subscriptions expiring within 30 minutes are renewed.
/// - Expired subscriptions are reported in the returned result.
pub async fn check_and_renew_subscriptions(
    ...
    Ok(SubscriptionCheckResult { renewed, expired })
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SubscriptionCheckResult {
    pub renewed: Vec<(String, String)>,
