...
use crate::encoding::decode_base64_standard;
...
pub fn parse_graph_message(
    ...
    // Dates: ISO 8601 to epoch milliseconds.
    ...
    let attachments: Vec<ParsedGraphAttachment> = msg
        ...
        .map(|a| {
            ...
            ParsedGraphAttachment {
                content_hash: inline_data.as_deref().map(BlobHash::hash),
                inline_data,
                id: a.id.clone(),
...
fn format_recipients(recipients: Option<&[GraphRecipient]>) -> Option<String> {
    ...
    let formatted: Vec<String> = recipients
        .iter()
        .map(|r| match r.email_address.name.as_deref() {
