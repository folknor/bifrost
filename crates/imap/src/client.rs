...
impl<T: Read + Write + Unpin + fmt::Debug + Send> Client<T> {
    ...
    ///
    /// ```ignore
    /// # fn main() -> bifrost_imap::error::Result<()> {
    /// # async_std::task::block_on(async {
    ///
    /// let tls = async_native_tls::TlsConnector::new();
    /// let client = bifrost_imap::connect(
    ...
    /// }
    ///
    /// impl bifrost_imap::Authenticator for &OAuth2 {
    ...
    ///     let tls = async_native_tls::TlsConnector::new();
    ///     let client = bifrost_imap::connect((domain, 993), domain, tls).await?;
...
mod tests {
    ...
    ///
    /// This is a regression test for v0.6.0 bifrost-imap,
