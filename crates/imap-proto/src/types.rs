...
pub enum AttributeValue<'a> {
    ...
    GmailMsgId(u64),
    GmailThrId(u64),
    /// RFC 8474 section 5.1 - `EMAILID`: a server-assigned unique identifier for
    ...
    EmailId(Cow<'a, str>),
    /// RFC 8474 section 5.2 - `THREADID`: a server-assigned identifier for the
    /// thread a message belongs to.  `None` corresponds to the wire-form
    /// `THREADID NIL`, which RFC 8474 section 5.2 mandates for messages that do
