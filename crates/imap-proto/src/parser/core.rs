...
pub fn quoted(i: &[u8]) -> IResult<&[u8], &[u8]> {
    ...
    while pos < i.len() {
        match i[pos] {
            ...
            byte if is_text_char(byte) => pos += 1,
            _ => {
                return Err(nom::Err::Error(nom::error::make_error(
                    &i[pos..],
                    nom::error::ErrorKind::Char,
...
pub fn literal(input: &[u8]) -> IResult<&[u8], &[u8]> {
    ...
    let (remaining, data) = take(count).parse(remaining)?;
...
// From RFC5234
//
// Real-world IMAP servers (Dovecot, Cyrus, and others) regularly emit 8-bit bytes
