...
// Latin-1 "Willkommen" with u-umlaut = 0xFC) must parse without failing,
...
fn test_resp_text_lossy_decode_8bit() {
    ...
    response.push(0xFC); // Latin-1 u-umlaut, not valid UTF-8
...
// non-UTF-8 locales (Turkish "Goruntu1" with umlauts is a common case from Outlook
...
fn test_body_structure_with_8bit_literal_filename() {
    // ("name" {8}\r\n<G>\xf6<r>\xfc<n><t>\xfc<1>) - "Goruntu1" with umlauts in Latin-1
    let mut response: Vec<u8> = Vec::new();
    ...
    match resp {
        Response::Fetch(_, f) => {
            ...
            let element = parser.search(|b: &BodyStructure| {
                matches!(b, BodyStructure::Basic { common, .. } if common.ty.ty == "APPLICATION")
            });
...
fn test_fetch_rfc8474_threadid_nil() {
    // RFC 8474 section 5.2: THREADID can be NIL for messages with no thread association.
...
fn test_fetch_unknown_savedate_fallback() {
    // RFC 8514 section 2: SAVEDATE is not yet typed first-class.  Confirm the
