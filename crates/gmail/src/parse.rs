...
pub fn parse_gmail_message(msg: &GmailMessage) -> ParsedGmailMessage {
    ...
    ParsedGmailMessage {
        base: ParsedMessageBase {
            ...
            date: internal_date,
            is_read: !msg.label_ids.iter().any(|label| label == "UNREAD"),
            is_starred: msg.label_ids.iter().any(|label| label == "STARRED"),
            is_replied,
            is_forwarded,
            body_html,
            body_text,
...
fn extract_body(part: &GmailPayload, mime_type: &str) -> Option<String> {
    if !is_amp_content_type(&part.mime_type)
...
fn collect_attachments(part: &GmailPayload, results: &mut Vec<ParsedAttachment>) {
        ...
        && let Some(attachment_id) = &body.attachment_id
    {
        ...
        let is_inline = content_disposition.is_some_and(|h| {
            h.value
                .trim_start()
                .get(..6)
        ...
        if has_filename || has_cid {
            ...
            let inline_data = if is_inline && !has_filename && part.mime_type.starts_with("image/")
            {
                body.data
                    ...
                    .filter(|data| data.len() <= MAX_INLINE_SIZE)
            } else {
                None
            };
            let content_hash = inline_data.as_deref().map(BlobHash::hash);
...
fn is_amp_content_type(mime_type: &str) -> bool {
    mime_type.eq_ignore_ascii_case("text/x-amp-html")
}

fn parse_single_address_header(raw: Option<&str>) -> (Option<String>, Option<String>) {
...
fn dedup_by_key<K, T, FKey, FMerge>(items: Vec<T>, key_for: FKey, mut merge: FMerge) -> Vec<T>
where
    K: Eq + Hash,
    FKey: Fn(&T) -> K,
...
fn prefer_non_placeholder_filename(
    existing: &mut String,
    new: &str,
...
fn prefer_missing_clone<T: Clone>(existing: &mut Option<T>, new: &Option<T>) {
